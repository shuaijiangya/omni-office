...
- OpenAPI、capabilities、README 和环境变量示例必须与发布版本一致。
...
- PostgreSQL 执行一致性备份；S3 开启版本控制/生命周期，备份模板目录和外部配置。
...
- 应用不会自动创建或修改数据库对象；发布前由运维人员确认人工维护的表结构可供当前版本使用。
- 应用回滚不执行任何数据库结构操作；数据库相关人工操作由独立运维流程负责。
